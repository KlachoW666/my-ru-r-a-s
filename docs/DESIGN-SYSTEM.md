...
| Палитра | тёмная графитовая + explosive orange `#f36a21` (SATCHEL) | нейтральный zinc в OKLCH |
...
> **Ребрендинг SATCHEL.** Акцент — оранжевый `#f36a21`, не лайм. Все токены
> перекрыты в [`assets/css/satchel-theme.css`](../public/assets/css/satchel-theme.css),
> который подключён последним; главный `index-BSsnhUcQ.css` не правился и всё
> ещё содержит лаймовые значения — они перебиваются слоем темы.
>
> Полная таблица «было → стало» и список намеренно сохранённого:
> [SATCHEL-REBRAND.md](SATCHEL-REBRAND.md).
>
> Ключевое, чего не было в прежней теме: семейство `--color-accent-*` задано
> в бандле **литералами**, а не через `rgba(var(--accent-rgb), α)`. Поэтому
> перекрыть одно `--accent-rgb` недостаточно — псевдонимы нужно перечислять
> явно, иначе свечение и фокус остаются цвета прежнего бренда.
>
> Палитра редкостей не менялась: оранжевый — цвет действия, цвет редкости —
> информация о предмете.

### Исторический бренд-акцент — лайм `#b2ff00`
