...
function nodes(v){return v==null?[]:[v,...(Array.isArray(v?.children)?v.children.flatMap(nodes):[])];}
...
test('inventory destination provides a recovery link',()=>{const c=component({rewardDestination:'inventory',inventoryIds:[10],winnings:1943},()=>{});assert.equal(nodes(c.render()).find(x=>x.tag==='a')?.props.href,'/wallet/withdraw');});
test('pending sale rejects a second click',async()=>{let resolve,calls=0;const c=component({rewardDestination:'inventory',inventoryIds:[10],winnings:330},()=>{calls++;return new Promise(r=>resolve=r);});const button=nodes(c.render()).find(x=>x.tag==='button');const first=button.props.onClick();await button.props.onClick();assert.equal(calls,1);assert.equal(nodes(c.render()).find(x=>x.tag==='button').props.disabled,true);resolve({status:'success',data:{payout:330,balance:1000}});await first;});
