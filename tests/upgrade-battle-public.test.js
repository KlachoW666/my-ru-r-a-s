...
function load(){
  ...
  const context={URLSearchParams,URL,console,setTimeout,clearTimeout,setInterval,clearInterval,
    ...
    window:{location:{search:'?mode=upgrade',assign:()=>{}}}};
...
for(const [amount,valid] of [[.99,false],[1,true],[1.01,true],[9999.99,true],[10000,true],[10000.01,false],[NaN,false],[Infinity,false]]){
 test('round amount boundary '+amount,()=>{const m=setup(async()=>{});m.state.value.roundBet=amount;m.state.value.selected=targets.map(x=>({...x,price:100000}));m.prepareCreate();assert.equal(!!m.state.value.confirmation,valid);});
}
test('exactly three targets required',()=>{for(const count of [2,3,4]){const m=setup(async()=>{});m.state.value.selected=Array.from({length:count},(_,i)=>({...targets[0],id:i}));m.prepareCreate();assert.equal(!!m.state.value.confirmation,count===3);}});
